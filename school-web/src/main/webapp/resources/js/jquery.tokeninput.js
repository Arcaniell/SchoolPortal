...
(function($) {
		...
		init : function(url_or_data_or_function, options) {
			...
			return this.each(function() {
				$(this)
						.data(
								"tokenInputObject",
								new $.TokenList(this, url_or_data_or_function,
										settings));
			});
		},
		clear : function() {
	...
	$.TokenList = function(input, url_or_data, settings) {
		...
		if ($.type(url_or_data) === "string"
			...
			if (settings.crossDomain === undefined) {
				if (url.indexOf("://") === -1) {
					...
					settings.crossDomain = (location.href.split(/\/+/g)[1] !== url
							.split(/\/+/g)[1]);
				}
			}
						...
						function(event) {
							...
							case KEY.DOWN:
								if (!$(this).val()) {
									...
									if ((previous_token.length && previous_token
											.get(0) === selected_token)
											|| (next_token.length && next_token
													.get(0) === selected_token)) {
									...
									if (event.keyCode === KEY.DOWN
										...
										dropdown_item = $(
												selected_dropdown_item).next();
									} else {
										dropdown_item = $(
												selected_dropdown_item).prev();
									}
							...
							case KEY.COMMA:
								if (selected_dropdown_item) {
									add_token($(selected_dropdown_item).data(
											"tokeninput"));
									hidden_input.change();
									return false;
								}
		...
		var token_list = $("<ul />").addClass(settings.classes.tokenList)
		...
		var input_token = $("<li />").addClass(settings.classes.inputToken)
				.appendTo(token_list).append(input_box);

		// The list to store the dropdown items in
		var dropdown = $("<div>").addClass(settings.classes.dropdown).appendTo(
				"body").hide();
		...
		function checkTokenLimit() {
			if (settings.tokenLimit !== null
					&& token_count >= settings.tokenLimit) {
				input_box.hide();
				hide_dropdown();
				return;
			}
		...
		function resize_input() {
			...
			var escaped = input_val.replace(/&/g, '&amp;').replace(/\s/g, ' ')
					.replace(/</g, '&lt;').replace(/>/g, '&gt;');
		...
		function insert_token(item) {
			...
			$("<span>" + settings.deleteText + "</span>").addClass(
					settings.classes.tokenDelete).appendTo(this_token).click(
					function() {
			...
			saved_tokens = saved_tokens.slice(0, selected_token_index).concat(
					[ token_data ]).concat(
			...
			if (token_count > 0 && settings.preventDuplicates) {
				...
				token_list.children().each(
						function() {
							...
							var existing_data = $.data(existing_token.get(0),
									"tokeninput");
		...
		function delete_token(token) {
			...
			saved_tokens = saved_tokens.slice(0, index).concat(
					saved_tokens.slice(index + 1));
			if (index < selected_token_index)
				selected_token_index--;
		...
		function populate_dropdown(query, results) {
			if (results && results.length) {
				...
				var dropdown_ul = $("<ul>").appendTo(dropdown)
						.mouseover(
								function(event) {
									select_dropdown_item($(event.target)
											.closest("li"));
								}).mousedown(
								function(event) {
				...
				$.each(results, function(index, value) {
					...
					this_li = find_value_and_highlight_term(this_li,
							value[settings.propertyToSearch], query);
				...
				if (settings.url) {
					...
					ajax_params.success = function(results) {
						if ($.isFunction(settings.onResult)) {
							results = settings.onResult.call(hidden_input,
									results);
						}
						cache
								.add(
										cache_key,
										settings.jsonContainer ? results[settings.jsonContainer]
												: results);
					...
					var results = $.grep(settings.local_data, function(row) {
						return row[settings.propertyToSearch].toLowerCase()
